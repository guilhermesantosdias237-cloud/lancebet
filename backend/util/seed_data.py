...
from util.foto_util import obter_path_absoluto_foto, foto_existe
...
def _email_admin_seed() -> Optional[str]:
    """E-mail do administrador definido em admin_seed.json (fonte única).

    Usado pelo seed de domínio para localizar o admin (autor dos eventos,
    avatar) sem hardcodear o e-mail — assim, trocar o admin em admin_seed.json
    não exige editar o código.
    """
    for usuario in _ler_usuarios_do_json():
        if usuario.get("perfil") == Perfil.ADMIN.value:
...
# Apostadores fictícios. O administrador vem de admin_seed.json (não duplicar).
...
EVENTOS_SEED = [
    ...
    },
    {
        ...
        "escudo_mandante": "escudo_bahia.png",
        "escudo_visitante": "escudo_vitoria.png",
        "opcoes": [
...
def carregar_lancebet_seed():
    ...
    O administrador e os apostadores são tratados por e-mail (não duplica).
    """
    with obter_conexao() as conn:
        ...
        # Admin: aplica avatar se o usuário do seed existir (e-mail vem do
        # admin_seed.json, não hardcodeado).
        email_admin = _email_admin_seed()
        admin_id = None
        if email_admin:
            cursor.execute("SELECT id FROM usuario WHERE email = ?", (email_admin,))
...
def garantir_avatares_seed():
    """Reaplica os avatares fictícios para usuários do seed cujo arquivo de foto
    esteja ausente.

    Necessário porque ``static/img/usuarios/`` é gerado em runtime (não vai para
    a imagem Docker — está no .gitignore) e o seed de domínio só roda quando não
    há eventos. Após um redeploy, o container nasce sem as fotos e o seed pula,
    deixando os avatares como 404. Esta função roda SEMPRE no boot, é idempotente
    e só age quando o arquivo está ausente — nunca sobrescreve uma foto enviada
    pelo próprio usuário (que, com o volume persistente, sobrevive ao redeploy).
    """
    avatares_por_email = {ap["email"]: ap["avatar"] for ap in APOSTADORES_SEED}
    email_admin = _email_admin_seed()
    if email_admin:
        avatares_por_email[email_admin] = "avatar_admin.png"

    reaplicados = 0
    with obter_conexao() as conn:
        cursor = conn.cursor()
        for email, arquivo in avatares_por_email.items():
            ...
            if foto_existe(usuario_id):
                continue
            if _aplicar_avatar(usuario_id, arquivo):
                reaplicados += 1

    if reaplicados:
        logger.info(f"Avatares de seed reaplicados (estavam ausentes): {reaplicados}")
...
def inicializar_dados():
    ...
    try:
        ...
        # DECISÃO DESTE FORK (diverge do starter): o seed de DEMO de domínio
        # (apostadores, eventos, apostas, carteiras, avatares) roda SEMPRE,
        # inclusive em PRODUÇÃO — por requisito do projeto, lancebet.ifes.site
        # deve exibir os dados de exemplo. O starter gateia isso por
        # IS_DEVELOPMENT (demo só em dev, ver docs/FORKING.md §10e); aqui esse
        # gate é intencionalmente dispensado. Ambas as funções são idempotentes
        # (guardas de tabela/arquivo), então rodar a cada boot é seguro.
        carregar_lancebet_seed()
        garantir_avatares_seed()
        logger.info("=" * 50)
