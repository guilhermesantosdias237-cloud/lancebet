...
class TestCadastro:
    ...
    def test_cadastro_ignora_perfil_injetado_cria_apostador(self, client):
        """Injetar `perfil` no auto-cadastro NÃO escala privilégio.

        Diferente do starter (que valida um campo `perfil` enviado pelo cliente
        e devolve 403), este fork usa CadastroApostadorDTO, que não tem campo
        `perfil`. Um `perfil` injetado no JSON é simplesmente ignorado pelo
        Pydantic e a conta é criada como Apostador — nunca como Administrador.
        """
        ...
        response = client.post(
            "/api/cadastrar",
            json={
                "perfil": Perfil.ADMIN.value,  # injeção: deve ser ignorada
        ...
        assert response.status_code == status.HTTP_201_CREATED
        criado = usuario_repo.obter_por_email("invasor@example.com")
