            ...
            <div key={u.id} style={{ display: 'grid', gridTemplateColumns: '1.6fr 1.6fr 1fr 1fr 0.8fr 1.2fr', gap: 12, padding: '14px 22px', borderBottom: '1px solid #F0F0F0', alignItems: 'center' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                {admin ? (
                  <button
                    disabled
                    title="O status do administrador não pode ser alterado"
                    style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.06em', padding: '4px 10px', textTransform: 'uppercase', cursor: 'not-allowed', opacity: 0.45, ...statusBadgeStyle(u.status) }}
                  >
                    {u.status}
                  </button>
                ) : (
