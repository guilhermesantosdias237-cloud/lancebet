// Detalhe do evento + boletim de aposta (rota /eventos/:id).
// Porte fiel de design/lancebet-react/src/pages/EventDetail.jsx, trocando placeBet
// do AppContext por POST /api/apostas. Retorno = round(valor * odd, 2) calculado no cliente.
import { useState } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { useFetch } from '../../hooks/useFetch'
import { useAuthStore } from '../../store/authStore'
import { useUIStore } from '../../store/uiStore'
import { apostasApi, eventosApi, ApiError } from '../../lib/api'
import { StatusEvento, StatusOpcao } from '../../lib/types'
import type { OpcaoAposta } from '../../lib/types'
import { fmt, ofmt } from '../../lib/format'
import { Button, Badge, HoverCard, statusEventoStyleDark, optionCardStyle } from '../../components/lancebet/ui'
...
export default function EventoDetalhePage() {
  ...
  const toast = useUIStore((s) => s.mostrarToast)
  const usuario = useAuthStore((s) => s.usuario)
  const setSaldo = useAuthStore((s) => s.setSaldo)

  const { data: ev, carregando, erro } = useFetch(() => eventosApi.obter(Number(id)), [id])
  ...
  const selOpt = ev.opcoes.find((o) => o.id === selectedOpt) ?? null
  ...
  const saldo = usuario?.saldo_ficticio ?? 0

  const pick = (o: OpcaoAposta) => {
    if (o.status !== StatusOpcao.ATIVA) {
      toast('Mercado suspenso.', 'warning')
      return
    }
    setSelectedOpt(o.id)
  }
  ...
  const confirm = async () => {
    if (!selOpt) {
      toast('Selecione uma opção.', 'warning')
      return
    }
    if (ev.status !== StatusEvento.ABERTO) {
      toast('Apostas encerradas para este evento.', 'warning')
      return
    }
    if (stakeN <= 0) {
      toast('Informe um valor para apostar.', 'warning')
      return
    }
    if (stakeN > saldo) {
      toast('Saldo insuficiente.', 'warning')
      return
    }
    setEnviando(true)
    try {
      const aposta = await apostasApi.criar({ opcao_aposta_id: selOpt.id, valor_apostado: stakeN })
      setSaldo(aposta.saldo_apos)
      navigate('/aposta-confirmada', { state: { aposta } })
    } catch (e) {
      if (e instanceof ApiError) toast(e.message, 'danger')
      else toast('Não foi possível registrar a aposta.', 'danger')
    } finally {
      setEnviando(false)
    }
  }
        ...
        <div>
          <div style={{ background: '#000', color: '#fff', padding: '34px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
              ...
              <Badge style={{ fontSize: 10, padding: '3px 8px', ...statusEventoStyleDark(ev.status) }}>{ev.status}</Badge>
            </div>
            ...
            <div style={{ fontSize: 13, color: '#B3B5B7', fontWeight: 600, marginTop: 16 }}>{ev.data_hora ?? 'A definir'}</div>
          </div>
          ...
          <div style={{ padding: '22px 20px' }}>
            {selOpt ? (
              <>
                ...
                <Button onClick={confirm} disabled={enviando} style={{ width: '100%', opacity: enviando ? 0.6 : 1 }}>{enviando ? 'Confirmando…' : 'Confirmar aposta'}</Button>
              </>
            ) : (
